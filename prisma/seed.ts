...
async function main() {
  ...
  const clientsData = [
    ...
    { name: 'Bakkerij Lievens', address: 'Brusselsesteenweg 88, 1790 Affligem', defaultPrice: 45, frequencyWeeks: 4, paymentMethod: 'cash', zoneId: zoneA.id, contactPerson: 'Marc Lievens', phone: '0476 23 14 88', email: 'marc@bakkerijlievens.be', notes: 'Toegang via zij-ingang. Niet bellen vóór 8u30. Vraagt altijd factuur voor boekhouding.' },
  ...
  for (const data of clientsData) {
    ...
    // Generate 6 months of visits
    const now = new Date();
    let visitDate = new Date(now);
    visitDate.setMonth(visitDate.getMonth() - 6);

    while (visitDate < now) {
      const isPending = visitDate > new Date(now.getTime() - 14 * 24 * 60 * 60 * 1000) && Math.random() < 0.25;
  ...
  console.log(`Seeded: ${clientCount} clients, ${visitCount} visits`);
}
